...
function sanitizeFileArgs(argv: string[]) {
  const invalidFile = path.extname(argv[0]) !== '.sql';
...
class PgAnonymizer extends Command {
  static description = "anonymized dump file";
  ...
  static args = [
    {
      name: "PGARG",
      description:
        "arguments to pass a dump file (in.sql)",
      required: false,
    },
  ];
  ...
  async run() {
    ...
    sanitizeFileArgs(argv);

    log.status();
    log.status("Reading your file dump...");

    const pgDumpFile = fs.readFileSync(argv[0]);

    log.status("File uploaded, running anonymization.");
